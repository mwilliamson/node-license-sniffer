...
function sniff(projectPath, options, callback) {
    if (!callback) {
        callback = options;
        options = {};
    }
...
function sniffLicenseFile(modulePath, callback) {
    sniffFile(path.join(modulePath, "LICENSE"), function(err, name) {
        if (err) {
            callback(err);
        } else if (name) {
            callback(null, {names: [name]});
        } else {
            sniffFile(path.join(modulePath, "UNLICENSE"), function(err, name) {
                if (err) {
                    callback(err);
                } else {
                    var names = name ? [name] : [];
