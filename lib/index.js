...
function sniffReadmeContents(contents) {
    var licenseText = findTextAfterHeader(contents, "License");
    return detectLicenseFromText(licenseText);
}

function findTextAfterHeader(contents, header) {
    var elements = markdown.markdown.parse(contents);
    
    for (var i = 0; i < elements.length && (elements[i][0] !== "header" || elements[i][2] !== header); i++) {
    }
    i++
    var text = [];
